...
#define enB 5
 ...
 int mediumhigh = basespeed + 50;
 int hardhigh = basespeed;
 int farhigh = basespeed +25;
 int softlow = basespeed -45;
 int mediumlow = basespeed -50;
 int hardlow = basespeed -80;
 int farlow = basespeed +25;
...
void loop() {
  ...
  int FL_SENSOR = digitalRead(A6);
  int FR_SENSOR = digitalRead(3);
  Serial.println(FL_SENSOR);
...
if (FL_SENSOR==1) {
  ...
  else if(HL_SENSOR==0 && ML_SENSOR==0 && SL_SENSOR==0 && SR_SENSOR==0 && MR_SENSOR==0 && HR_SENSOR==0) {
    lastdirection();
}
...
void farright()
{
            ...
            analogWrite(enA, farhigh);
            analogWrite(enB, farlow);
            lastdir = 1;
}
...
void farleft()
{
            ...
            analogWrite(enA, farlow);
            analogWrite(enB, farhigh);
}
